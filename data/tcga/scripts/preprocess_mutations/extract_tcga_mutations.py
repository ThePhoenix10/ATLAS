...
WANTED_COLUMNS = ['Hugo_Symbol', 'Variant_Classification', 'Variant_Type', 'Tumor_Sample_Barcode', 't_depth', 't_alt_count', 't_ref_count', 'case_id']
NON_SILENT_CLASSES = {'Missense_Mutation', 'Nonsense_Mutation', 'Frame_Shift_Del', 'Frame_Shift_Ins', 'In_Frame_Del', 'In_Frame_Ins', 'Splice_Site', 'Nonstop_Mutation', 'Translation_Start_Site'}
...
def build_and_save_matrix(numeric_records: list, output_dir: Path, case_id_to_tumor_origin: dict=None) -> tuple:
    import numpy as np
    ...
    sparse_matrix = coo_matrix((values, (row_idx, col_idx)), shape=(len(case_ids), len(col_names))).tocsr()
    ...
    with open(cols_path, 'w') as f:
        f.write('\n'.join(col_names))
    log.info(f'Saved sparse patient mutation matrix ({sparse_matrix.shape[0]} patients x {sparse_matrix.shape[1]} columns, {sparse_matrix.nnz} non-zero entries) to:\n  {npz_path} (load with scipy.sparse.load_npz)\n  {rows_path} (tumor_origin<TAB>case_id per row)\n  {cols_path} (column name per column)')

    return sparse_matrix.shape

def cleanup_empty_maf_patients(mutations: pd.DataFrame, matched_manifest_path: Path, matched_cohort_csv_path: Path, embeddings_dir: Path, non_silent_only: bool):
    ...
    log.info(f'Empty-MAF cleanup: {len(all_barcodes)} total matched patients, {len(all_barcodes) - len(empty_barcodes)} have qualifying mutation data, {len(empty_barcodes)} have none (empty MAF or silent-only; will be removed)')
...
def main():
    ...
    parser.add_argument('--matched-manifest', required=True, help='Manifest listing the matched-cohort file_ids to process')
    parser.add_argument('--output-dir', default='/workspace/data/tcga/genomics/mutations', help='Where to write output tables')
    parser.add_argument('--non-silent-only-in-matrix', action='store_true', default=True, help='Restrict the mutation matrix to non-silent mutations (default: on)')
    parser.add_argument('--matched-cohort-csv', default=None, help='Path to matched_cohort.csv from the matching step. If provided (along with --embeddings-dir), empty-MAF patients are removed from it too.')
    parser.add_argument('--embeddings-dir', default=None, help="Path to the extracted .h5 embeddings folder. If provided (along with --matched-cohort-csv), empty-MAF patients' .h5 files are moved to embeddings_excluded/ next to this folder.")
    ...
    for i, maf_path in enumerate(tqdm(maf_files, desc='Reading MAFs', unit='file'), 1):
        ...
        try:
            ...
            all_rows.append(df)
            numeric_records = build_patient_records(df, args.non_silent_only_in_matrix)
        ...
        if i % 1000 == 0:
            log.info(f'[{i}/{len(maf_files)}] MAF files processed, {len(all_numeric_records)} matrix records so far')
    ...
    if args.matched_cohort_csv and args.embeddings_dir:
        n_empty_removed = cleanup_empty_maf_patients(mutations, Path(args.matched_manifest), Path(args.matched_cohort_csv), Path(args.embeddings_dir), args.non_silent_only_in_matrix)
    else:
    ...
    matrix_shape = build_and_save_matrix(all_numeric_records, output_dir, case_id_to_tumor_origin)
    ...
    with open(summary_path, 'w') as f:
        ...
        if matrix_shape and matrix_shape[0]:
            f.write(f'\nPatient mutation matrix shape: {matrix_shape[0]} patients x {matrix_shape[1]} columns (saved as sparse .npz, see log for file paths)\n')
        if failed_files:
