...
def map_primary_site(primary_site):
    ...
    if 'uterus' in text or 'endometrium' in text:
        return 'Uterus'
    if any((x in text for x in ['larynx', 'tongue', 'oral cavity', 'pharynx', 'tonsil', 'mouth', 'lip'])):
...
def query_batch(file_ids, timeout=120, retries=5):
    payload = {'filters': {'op': 'in', 'content': {'field': 'files.file_id', 'value': file_ids}}, 'format': 'JSON', 'size': len(file_ids), 'fields': ','.join(['file_id', 'file_name', 'cases.case_id', 'cases.submitter_id', 'cases.project.project_id', 'cases.primary_site', 'cases.disease_type'])}
    last_error = None
    ...
    for start in tqdm(range(0, len(ids), batch_size), desc='Querying GDC CNA metadata', unit='batch'):
        for hit in query_batch(ids[start:start + batch_size]):
            ...
            for case in hit.get('cases', []) or []:
                ...
                records.append({'file_id': file_id, 'file_name': file_name, 'case_id': clean(case.get('case_id')), 'case_submitter_id': clean(case.get('submitter_id')), 'project_id': clean(project.get('project_id')), 'primary_site': clean(case.get('primary_site')), 'disease_type': clean(case.get('disease_type'))})
...
def load_training_cna_columns(path):
    full_columns = [line.strip() for line in Path(path).read_text().splitlines() if line.strip().startswith(CNA_PREFIX)]
...
def main():
    ...
    parser.add_argument('--output-dir', default='/workspace/data/cptac/genomics/cna')
    ...
    training_cna_full, training_base_map = load_training_cna_columns(args.align_to_columns_file)
    training_bases = set(training_base_map)
    print(f"Loaded {len(training_cna_full)} training CNA columns ({len(training_bases)} unique genes) using prefix '{CNA_PREFIX}'")

    if not training_cna_full:
        print(f"WARNING: zero training columns matched prefix '{CNA_PREFIX}' -- check --align-to-columns-file actually contains CNA columns, and that they use this prefix. Sample of first 5 lines from that file for reference:")
        sample = Path(args.align_to_columns_file).read_text().splitlines()[:5]

        for line in sample:
            print(f'  {line}')
    wanted_ids = set(manifest['id'].astype(str))
    cna_files = sorted((p for p in cna_dir.rglob('*.tsv') if p.parent.name in wanted_ids and 'gene_level.copy_number_variation' in p.name and ('/logs/' not in str(p))))
    print(f'Manifest records: {len(manifest)}')
    print(f'Matching gene-level CNA TSV files: {len(cna_files)}')
    metadata = get_metadata(manifest, output_dir / 'gdc_cna_metadata_cache.csv', refresh=args.refresh_metadata)
    file_to_case = {}
    case_meta = {}
    ...
    for _, row in metadata.iterrows():
        ...
        if cid:
            case_meta[cid] = {'case_id': cid, 'case_submitter_id': clean(row.get('case_submitter_id')), 'project_id': clean(row.get('project_id')), 'primary_site': clean(row.get('primary_site')), 'disease_type': clean(row.get('disease_type')), 'tumor_origin': map_primary_site(row.get('primary_site'))}
    ...
    for path in tqdm(cna_files, desc='Extracting CPTAC CNA', unit='file'):
        ...
        if not case_id:
            ...
            continue
        try:
            df = pd.read_csv(path, sep='\t', usecols=['gene_id', 'gene_name', 'chromosome', 'copy_number'], dtype={'gene_id': str, 'gene_name': str, 'chromosome': str})
            ...
            autosomal = df[df['chromosome'].str.replace('chr', '', regex=False).isin([str(i) for i in range(1, 23)])]
            ...
            for row in df.itertuples(index=False):
                base = choose_training_identifier(row.gene_name, row.gene_id, training_bases)
            ...
            qc.append({'file_id': file_id, 'filename': path.name, 'case_id': case_id, 'rows': len(df), 'estimated_integer_ploidy': sample_ploidy, 'amp_genes': state_counts['amp'], 'gain_genes': state_counts['gain'], 'del_genes': state_counts['del'], 'loss_genes': state_counts['loss'], 'unmatched_training_identifiers': unmatched_identifiers, 'status': 'processed'})
        except Exception as error:
            failures.append((file_id, str(error)))
            qc.append({'file_id': file_id, 'filename': path.name, 'case_id': case_id, 'status': 'failed', 'error': str(error)})
    ...
    matrix = coo_matrix((np.ones(len(rows), dtype=np.int8), (np.asarray(rows), np.asarray(cols))), shape=(len(case_ids), len(columns)), dtype=np.int8).tocsr()
    ...
    for case_id in case_ids:
        ...
        record.update({'case_id': case_id, 'cna_file_count': file_counts[case_id], 'amplification_feature_count': gain_counts[case_id], 'deep_deletion_feature_count': loss_counts[case_id], 'total_cna_feature_count': len(patient_features[case_id])})
    ...
    origin_counts = Counter((case_meta.get(case_id, {}).get('tumor_origin', 'Unknown') for case_id in case_ids))
    lines = ['CPTAC CNA EXTRACTION SUMMARY', '=' * 70, f'Manifest records: {len(manifest)}', f'Matching CNA TSV files: {len(cna_files)}', f'Unique patients: {len(case_ids)}', f'CNA features: {len(columns)}', f'Matrix nonzero entries: {matrix.nnz}', f'Failed files: {len(failures)}', '', 'Tumor origins:']
