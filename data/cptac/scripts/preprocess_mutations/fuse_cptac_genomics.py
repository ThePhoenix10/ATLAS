...
def main():
    parser = argparse.ArgumentParser(
        description=(
            "Fuse CPTAC mutation, SBS96, and CNA features and align them "
            "to the TCGA training feature schema."
        )
    )
    ...
    mut, mut_cases, mut_cols, mut_labels = load_table(
        mutation_dir,
        "patient_mutation_matrix",
    )

    sig, sig_cases, sig_cols, sig_labels = load_table(
        signature_dir,
        "patient_signature_matrix",
    )
    ...
    sig_cols = [
        col
        if col.startswith(SIGNATURE_PREFIX)
        else f"{SIGNATURE_PREFIX}{col}"
        for col in sig_cols
    ]

    metadata = load_metadata(
        mutation_dir / "patient_metadata.csv"
    )

    cna_metadata = load_metadata(
        cna_dir / "patient_metadata.csv"
    )
    ...
    mut_index = {
        case_id: i
        for i, case_id in enumerate(mut_cases)
    }

    sig_index = {
        case_id: i
        for i, case_id in enumerate(sig_cases)
    }

    cna_index = {
        case_id: i
        for i, case_id in enumerate(cna_cases)
    }
    ...
    if not included:
        raise RuntimeError(
            "No patients have mutation + SBS96 + CNA data."
        )

    mut_block = np.zeros(
        (len(included), len(mut_cols)),
        dtype=np.float32,
    )

    sig_block = np.zeros(
        (len(included), len(sig_cols)),
        dtype=np.float32,
    )

    cna_block = np.full(
        (len(included), len(cna_cols)),
        np.nan,
        dtype=np.float32,
    )
    ...
    for row_idx, case_id in enumerate(included):
        mut_block[row_idx] = (
            mut[mut_index[case_id]]
            .toarray()
            .reshape(-1)
        )

        sig_block[row_idx] = (
            sig[sig_index[case_id]]
            .toarray()
            .reshape(-1)
        )

        cna_block[row_idx] = (
            cna[cna_index[case_id]]
            .toarray()
            .reshape(-1)
        )
    ...
    save_npz(
        ...
        csr_matrix(
            aligned.astype(np.float32).values
        ),
    )

    aligned.to_csv(
        output_dir / "cptac_matrix_dense.csv"
    )

    (
        output_dir / "cptac_matrix_columns.txt"
    ).write_text(
        "\n".join(training_columns)
    )

    with (
        output_dir / "cptac_matrix_rows.txt"
    ).open("w") as handle:
    ...
    report = [
        ...
        (
            "Absent mutation columns set to 0: "
            f"{counts['missing_mut_zero']}"
        ),
        (
            "Absent SBS96 columns set to 0: "
            f"{counts['missing_sig_zero']}"
        ),
        (
            "Absent CNA columns set to 0: "
            f"{counts['missing_cna_zero']}"
        ),
        (
            "Other absent columns set to 0: "
            f"{counts['other_zero']}"
        ),
    ]

    (
        output_dir / "cptac_fusion_report.txt"
    ).write_text(
        "\n".join(report)
    )
