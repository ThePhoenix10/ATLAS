...
DEFAULT_FUSED_ROWS = '/workspace/data/tcga/genomics/fused/patient_fused_intogen_filtered_rows.txt'
DEFAULT_CASE_MAP = '/workspace/data/tcga/metadata/case_id_barcode_map.csv'
DEFAULT_LABELS_CSV = '/workspace/data/tcga/multimodal/metadata/matched_patients.csv'
DEFAULT_OUTPUT = '/workspace/results/spatial-aware'
DEFAULT_SPLIT_DIR = '/workspace/data/tcga/multimodal/splits'
...
class SpatialGraphDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if genomic_x is None:
            raise KeyError(f"'genomic_x' missing from multimodal graph {graph_path}")
        ...
        return {'x': x, 'edge_index': edge_index, 'edge_attr': edge_attr, 'genomic_x': genomic_x, 'y': y, 'patient_id': row['patient_norm']}
...
def collate_graphs(batch):
    ...
    for graph_id, item in enumerate(batch):
        ...
        ys.append(item['y'])
        genomic_xs.append(item['genomic_x'])
    ...
    genomic_x = torch.stack(genomic_xs, dim=0)

    return {'x': x, 'edge_index': edge_index, 'edge_attr': edge_attr, 'batch': batch_tensor, 'genomic_x': genomic_x, 'y': y, 'patient_ids': patient_ids}
...
class GenomicEncoder(nn.Module):
    ...
    def __init__(self, genomic_input_dim, hidden_dim, intermediate_dim=512, dropout=0.1):
        super().__init__()
        self.encoder = nn.Sequential(nn.LayerNorm(genomic_input_dim), nn.Dropout(min(0.15, dropout + 0.05)), nn.Linear(genomic_input_dim, intermediate_dim), nn.GELU(), nn.LayerNorm(intermediate_dim), nn.Dropout(dropout), nn.Linear(intermediate_dim, hidden_dim), nn.GELU(), nn.LayerNorm(hidden_dim), nn.Dropout(dropout))

    def forward(self, genomic_x):
        return self.encoder(genomic_x)

class SpatialGATKAN(nn.Module):

    def __init__(self, input_dim, hidden_dim, num_classes, genomic_input_dim=7146, genomic_intermediate_dim=512, num_layers=3, dropout=0.1, num_heads=4, kan_grid_size=8, radius_multiplier=1.5, kan_rank=16, kan_frequencies=4):
        super().__init__()
        self.genomic_encoder = GenomicEncoder(genomic_input_dim=genomic_input_dim, hidden_dim=hidden_dim, intermediate_dim=genomic_intermediate_dim, dropout=dropout)
        ...
        self.multimodal_interaction = LowRankFourierKAN(input_dim=fusion_dim, output_dim=hidden_dim, rank=kan_rank, num_frequencies=kan_frequencies, dropout=dropout)
        self.multimodal_gate = nn.Sequential(nn.Linear(hidden_dim * 2, hidden_dim), nn.GELU(), nn.Linear(hidden_dim, hidden_dim), nn.Sigmoid())
        nn.init.zeros_(self.multimodal_gate[2].weight)
        nn.init.constant_(self.multimodal_gate[2].bias, -2.0)
        self.multimodal_norm = nn.LayerNorm(hidden_dim)
        self.classifier = nn.Linear(hidden_dim, num_classes)

    def forward(self, x, edge_index, edge_attr, batch, genomic_x):
        genomic_context = self.genomic_encoder(genomic_x)
        x = self.input_proj(x)
        ...
        pathology_state = self.graph_readout(graph_state)
        pathology_state = self.readout_norm(pathology_state)
        interaction_features = torch.cat([pathology_state, genomic_context, pathology_state * genomic_context, torch.abs(pathology_state - genomic_context)], dim=1)
        interaction_delta = self.multimodal_interaction(interaction_features)
        gate = self.multimodal_gate(torch.cat([pathology_state, genomic_context], dim=1))
        multimodal_state = pathology_state + gate * interaction_delta
        multimodal_state = self.multimodal_norm(multimodal_state)

        return self.classifier(multimodal_state)
...
def move_batch(batch, device):
    return {'x': batch['x'].to(device, non_blocking=True), 'edge_index': batch['edge_index'].to(device, non_blocking=True), 'edge_attr': batch['edge_attr'].to(device, non_blocking=True), 'batch': batch['batch'].to(device, non_blocking=True), 'genomic_x': batch['genomic_x'].to(device, non_blocking=True), 'y': batch['y'].to(device, non_blocking=True), 'patient_ids': batch['patient_ids']}
...
def train_one_epoch(model, loader, optimizer, criterion, scaler, device, use_amp, num_classes):
    ...
    for batch in pbar:
        ...
        with torch.autocast(device_type='cuda', dtype=torch.float16, enabled=use_amp):
            logits = model(batch['x'], batch['edge_index'], batch['edge_attr'], batch['batch'], batch['genomic_x'])
...
def save_combined_five_fold_curves(output_dir, n_folds=5, condition='Spatial-Aware'):
    output_dir = Path(output_dir)
...
def main():
    ...
    parser.add_argument('--genomic_intermediate_dim', type=int, default=512, help='Intermediate width of the internal genomic encoder. Default: 7146 -> 512 -> hidden_dim.')
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    print('=' * 100)
    print('ATLAS HIERARCHICAL MULTIMODAL SPATIAL KA-GAT: GATED GLOBAL GENOMICS')
    print('PRE-GENERATED SITE-STRATIFIED 5-FOLD CROSS-VALIDATION')
    print('=' * 100)
    ...
    print()
    print('=' * 100)
    print('USING PRE-GENERATED MATCHED SITE-STRATIFIED 5-FOLD SPLITS')
    print('=' * 100)
    print('Split directory:', args.split_dir)
    print('Assignments are reused exactly from the pathology-only experiment.')
    ...
    for fold in folds_to_run:
        ...
        for local_df in (train_df, val_df, test_df):
            local_df['graph_path'] = local_df['patient_norm'].map(lambda x: str(graph_map[normalize_patient_id(x)]))
        ...
        genomic_input_dim = sample['genomic_x'].numel()
        print('Input pathology embedding dim:', input_dim)
        print('Input genomic dim:', genomic_input_dim)
        model = SpatialGATKAN(input_dim=input_dim, hidden_dim=args.hidden_dim, num_classes=num_classes, genomic_input_dim=genomic_input_dim, genomic_intermediate_dim=args.genomic_intermediate_dim, num_layers=args.num_layers, dropout=args.dropout, num_heads=args.num_heads, kan_grid_size=args.kan_grid_size, radius_multiplier=args.radius_multiplier, kan_rank=args.kan_rank, kan_frequencies=args.kan_frequencies).to(device)
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if val_result['top1'] > best_top1:
                ...
                best_epoch = epoch
                epochs_without_improvement = 0
                torch.save({'model_state_dict': model.state_dict(), 'epoch': epoch, 'best_val_top1': best_top1, 'input_dim': input_dim, 'genomic_input_dim': genomic_input_dim, 'genomic_intermediate_dim': args.genomic_intermediate_dim, 'hidden_dim': args.hidden_dim, 'num_layers': args.num_layers, 'num_heads': args.num_heads, 'kan_grid_size': args.kan_grid_size, 'radius_multiplier': args.radius_multiplier, 'kan_rank': args.kan_rank, 'kan_frequencies': args.kan_frequencies, 'dropout': args.dropout, 'architecture': 'ATLAS_Hierarchical_Global_GatedResidual_Multimodal_SpatialKAGAT_FourierKAN', 'classes': label_encoder.classes_.tolist()}, best_path)
    ...
    if args.only_fold is not None:
        ...
        print()
        print('=' * 100)
        print('SITE-STRATIFIED 5-FOLD CROSS-VALIDATION COMPLETE')
        print('=' * 100)
    ...
    if args.only_fold is None:
        print()
        print('=' * 100)
        print('GENERATING FINAL COMBINED 5-FOLD LEARNING CURVES')
        print('=' * 100)
